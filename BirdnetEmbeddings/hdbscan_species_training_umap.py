...
def hdbscan_model(embeddings:pd.DataFrame, embeddingColumns:list):
    ...
    for species in unique_species:
        if (min(5, len(embeddings[embeddings["FILE SPECIES"] == species].index))) <= 1:
            continue
        model = HDBSCAN(min_cluster_size = min(50, len(embeddings[embeddings["FILE SPECIES"] == species].index)),
            min_samples = min(10, len(embeddings[embeddings["FILE SPECIES"] == species].index)),
