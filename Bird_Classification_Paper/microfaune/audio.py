...
def wav2spc(wav_file, fs=44100, n_mels=40, n_fft=2048, hop_len=1024, duration=None):
    ...
    # Checking if the clip is empty
#    if x.size == 0:
#        
    if duration is not None:
        x = x[:int(x_fs * duration) + 1]
    # Resampling if a clip fails to be 44.1 kHz
    if x_fs != fs and x.size != 0:
        Resample_Rate = 44100/x_fs
        # Using Scipy's built-in resample function
        # Prefer over decimate since there is the possibility of training on lower and higher sample rate data
        x = signal.resample(x,int(len(x)*Resample_Rate))
